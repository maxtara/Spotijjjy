...
class SpotifyPlaylistUpdater:
    ...
    def __init__(self, config):
         ...
         self.playlist_id = config['__PLAYLIST_ID__']
    
    def connect_prompt():
        """
        Connects to spotify using prompt
        """
        token = util.prompt_for_user_token(self.username, scope=self.spotify_scope, client_id=self.client_id, client_secret=self.client_secret, redirect_uri=self.redirect_url)
    ...
    def find_one_song_spotify(self, song_pair, cutoff=0.6, search_include_first_artist=False, do_general_search=False , verbose=False):
        ...
        for searched_song in searched_songs:
            ...
            for searched_artist in searched_artists:
                ...
                if score > 0:
                    if verbose:
                        print("FOUND - " +str(song_pair) + " ==== " +  searched_artist_name + ":" + searched_song['name'])
